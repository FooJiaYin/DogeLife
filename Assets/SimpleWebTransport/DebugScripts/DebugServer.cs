        ...
        public void OnGUI()
        {
            using (new GUILayout.AreaScope(new Rect(guiPosition, new Vector2(500, 500))))
            {
                if (GUILayout.Button("Send Message"))
                {
                    ...
                    transport.ServerSend(1, new ArraySegment<byte>(new byte[] { 1, 2, 4, 8 }), Channels.Reliable);
#else
                    transport.ServerSend(new System.Collections.Generic.List<int>() { 1 }, new ArraySegment<byte>(new byte[] { 1, 2, 4, 8 }), Channels.Reliable);
#endif
                }
            }
        }
